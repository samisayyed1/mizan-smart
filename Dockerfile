...
# Mizan Connect configuration (baked into JS bundle at build time)
...
# Mizan Connect configuration (baked into server binary at build time)
ARG CONNECT_AUTH_URL=
ARG CONNECT_AUTH_PUBLISHABLE_KEY=
ENV CONNECT_AUTH_URL=${CONNECT_AUTH_URL}
ENV CONNECT_AUTH_PUBLISHABLE_KEY=${CONNECT_AUTH_PUBLISHABLE_KEY}
...
RUN xx-cargo build --release --manifest-path apps/server/Cargo.toml && \
    ...
    cp target/$(xx-cargo --print-target-triple)/release/mizan-server /mizan-server

# Final stage
FROM alpine:3.19
WORKDIR /app
# Copy from backend (which is now build platform, but binary is target platform)
COPY --from=backend /mizan-server /usr/local/bin/mizan-server
COPY --from=frontend /web-dist ./dist
ENV WF_DB_PATH=/data/mizan.db
# Mizan Connect API URL (can be overridden at runtime via -e or docker-compose)
ARG CONNECT_API_URL=
ENV CONNECT_API_URL=${CONNECT_API_URL}
VOLUME ["/data"]
EXPOSE 8080
CMD ["/usr/local/bin/mizan-server"]
