...
/**
 * Store Mizan Connect tokens in the backend's encrypted secret store.
...
/**
 * Clear Mizan Connect session from the backend's secret store.
 * Works in both desktop (Tauri) and web modes.
 */
/**
 * Restore Mizan Connect session from the backend (web mode only).
