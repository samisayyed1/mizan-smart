...
use mizan_core::sync::APP_SYNC_TABLES;
...
fn transport_err_from_sync(e: mizan_device_sync::DeviceSyncError) -> TransportError {
    TransportError {
        message: e.to_string(),
        retry_class: e.retry_class(),
        error_code: e.error_code().map(|s| s.to_string()),
        details: match &e {
            mizan_device_sync::DeviceSyncError::Api { details, .. } => details.clone(),
            _ => None,
        },
    }
}
use mizan_storage_sqlite::sync::{SqliteSyncEngineDbPorts, SyncTableRowCount};
...
fn get_sync_identity_from_store(state: &AppState) -> Option<SyncIdentity> {
    let raw = state
        ...
        .ok()
        .flatten()?;
    let identity: mizan_device_sync::SyncIdentity = serde_json::from_str(&raw).ok()?;
    Some(SyncIdentity {
...
fn decode_snapshot_sqlite_payload(
    ...
    let dek = mizan_device_sync::crypto::derive_dek(root_key, key_version as u32)
        .map_err(|e| format!("Failed to derive snapshot DEK: {}", e))?;
    let decrypted = mizan_device_sync::crypto::decrypt(&dek, blob_text.trim())
...
impl OutboxStore for ServerEnginePorts {
    async fn list_pending_outbox(
        &self,
        limit: i64,
    ) -> Result<Vec<mizan_core::sync::SyncOutboxEvent>, String> {
...
impl ReplayStore for ServerEnginePorts {
    ...
    async fn get_engine_status(&self) -> Result<mizan_core::sync::SyncEngineStatus, String> {
...
impl SyncTransport for ServerEnginePorts {
    async fn get_events_cursor(
        ...
        token: &str,
        device_id: &str,
    ) -> Result<mizan_device_sync::SyncCursorResponse, TransportError> {
        create_client()
...
pub async fn get_pairing_source_status(
    ...
    let sync_state = client
        ...
        .await
        .map_err(|e| e.to_string())?;
    if sync_state.trust_state != mizan_device_sync::TrustState::Trusted {
...
async fn snapshot_satisfies_freshness_gate(
    ...
    token: &str,
    device_id: &str,
    latest: &mizan_device_sync::SnapshotLatestResponse,
    min_created_at: &str,
) -> Result<bool, String> {
    let latest_created_at = mizan_device_sync::parse_sync_datetime_to_utc(&latest.created_at)
        .map_err(|e| format!("Invalid snapshot created_at in metadata: {}", e))?;
    let min_created_at = mizan_device_sync::parse_sync_datetime_to_utc(min_created_at)
...
pub async fn sync_bootstrap_snapshot_if_needed(
    ...
    let min_snapshot_created_at = match raw_freshness_gate {
        Some(value) => match mizan_device_sync::normalize_sync_datetime(&value) {
...
pub async fn generate_snapshot_now(
    ...
    let upload_headers = mizan_device_sync::SnapshotUploadHeaders {
