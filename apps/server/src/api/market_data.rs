...
use mizan_core::portfolio::{snapshot::SnapshotRecalcMode, valuation::ValuationRecalcMode};
...
) -> ApiResult<Json<mizan_core::quotes::ResolvedQuote>> {
    let inst_type = q
        .instrument_type
        .as_deref()
...
async fn get_exchanges() -> Json<Vec<ExchangeInfo>> {
    Json(mizan_market_data::get_exchange_list())
}
