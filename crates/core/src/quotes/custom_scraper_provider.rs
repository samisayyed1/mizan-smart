...
use mizan_market_data::errors::MarketDataError;
...
impl CustomScraperProvider {
    ...
    fn expand_url(
        ...
        let isin_owned: Option<String> = context.and_then(|ctx| match &ctx.instrument {
            mizan_market_data::InstrumentId::Bond { isin } => Some(isin.as_ref().to_string()),
            _ => None,
        });

        let mic_owned: Option<String> = context.and_then(|ctx| match &ctx.instrument {
            mizan_market_data::InstrumentId::Equity { mic, .. } => {
