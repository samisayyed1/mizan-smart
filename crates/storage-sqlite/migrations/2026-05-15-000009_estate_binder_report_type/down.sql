...
WHERE section_id IN (
  ...
  WHERE report_runs.report_type = 'estate_binder'
);

DELETE FROM report_sections
WHERE report_run_id IN (
  SELECT id FROM report_runs WHERE report_type = 'estate_binder'
);

DELETE FROM report_runs WHERE report_type = 'estate_binder';
