...
/// `mizan_core::Error` before being returned to callers.
#[derive(Error, Debug)]
...
pub fn map_pool_err<T>(result: std::result::Result<T, r2d2::Error>) -> mizan_core::Result<T> {
    result.map_err(|e| e.into_core_error())
}
...
pub trait IntoCore<T> {
    fn into_core(self) -> mizan_core::Result<T>;
}
