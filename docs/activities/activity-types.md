...
Mizan. Understanding these types is essential for tracking portfolio movements
and for creating activities programmatically through addons or CSV imports.

## Overview

Mizan uses a closed set of **14 canonical activity types**. Each activity
...
**Purpose**: Incoming funds from outside Mizan (external source).
...
- Default: Transfer between Mizan accounts (cost basis preserved)
...
- Default: Transfer between Mizan accounts
