...
Update the `@mizan/addon-sdk` dependency and devDependencies:
...
Mizan v3 uses Tailwind CSS v4. While the addon system uses the host app's
...
import { Button, Card, Page } from "@mizan/ui";
```
...
2. **Test in development mode**: Run with mizan with
