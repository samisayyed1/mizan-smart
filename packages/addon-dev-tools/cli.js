...
async function createAddon(name, options) {
  try {
    ...
    if (interactive && (!config.description || !config.author)) {
      ...
      try {
        if (!config.description) {
          const defaultDesc = `A Mizan addon for ${name}`;
    ...
    if (!config.description) {
      config.description = `A Mizan addon for ${name}`;
    }
    if (!config.author) {
...
// CLI Setup
program.name("mizan").description("Mizan Addon Development CLI").version("1.0.0");
